...
class ShapePointsMesh extends THREE.Points{
    ...
    _updateGeometry(){
        ...
        if( this._verts.length  > bVerts.array.length || 
            ...
            this.geometry   = _newShapePointsMeshGeometry( this._verts, this._color, this._conifg );
            this._dirty     = false;
            return;
        }
...
function newShapePointsMeshMaterial(){
    ...
    void main(){
        ...
        if( fragShape == 6 ) alpha = ring();
        if( fragShape == 7 ) alpha = sdStar( 1.0, 3, 2.3 );
        if( fragShape == 8 ) alpha = sdStar( 1.0, 6, 2.5 );
        if( fragShape == 9 ) alpha = sdStar( 1.0, 4, 2.4 );
        if( fragShape == 10 ) alpha = sdStar( 1.0, 5, 2.8 );
