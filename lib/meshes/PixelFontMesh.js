...
class PixelFontMesh extends THREE.Mesh{
    ...
    constructor( props={} ){
        super( 
            ...
            PixelFontMaterial( props ),
        );

        if( props.res ) this.setRes( props.res[0],  props.res[1] );

        this.onBeforeRender = ()=>{ this.update() };
    }

    setColor( c ){ this.material.uniforms.color.value.set( c ); return this; }
    setAlpha( v ){ this.material.uniforms.alpha.value = v; return this; }
...
function PixelFontMaterial( props={} ){
    props = Object.assign({
        color : 'white',
        alpha : 1,
    }, props )
    ...
    */
    ...
    void main(){
        ...
        out_color   = vec4( color * mask, ( alpha > 0.5 )? 1.0 : mask );
    }`});


    // Object.defineProperty( mat, 'color', {
    //     set( v ){ mat.uniforms.color.value.set( v ); }
    // });

    // Object.defineProperty( mat, 'alpha', {
    //     set( v ){ mat.uniforms.alpha.value = v; }
    // });
