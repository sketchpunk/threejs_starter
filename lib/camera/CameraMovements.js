...
// rotate angle = 2 * Math.PI * delta / canvasHeight

// const position = scope.object.position;
// offset.copy( position ).sub( scope.target );
// let targetDistance = offset.length();
// half of the fov is center to top of screen
// targetDistance *= Math.tan( ( scope.object.fov / 2 ) * Math.PI / 180.0 ); // proejction
...
export function sphericalLook( camera, lon, lat, radius, target = [0, 0, 0] ){
    const pos   = [0, 0, 0];
